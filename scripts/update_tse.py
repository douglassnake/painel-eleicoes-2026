...
def resources():
    j=requests.get(CKAN,headers=UA,timeout=40).json()['result']['resources']
    cand=bens=None
    for x in j:
        label=norm((x.get('name') or '')+' '+(x.get('description') or ''))
        if 'BENS DE CANDIDATOS' in label or ('BENS' in label and 'CANDIDAT' in label): bens=x['url']
        elif 'CANDIDATOS' in label and 'COMPLEMENT' not in label and 'BENS' not in label and not cand: cand=x['url']
    if not cand: cand='https://cdn.tse.jus.br/estatistica/sead/odsele/consulta_cand/consulta_cand_2026.zip'
    if not bens: bens='https://cdn.tse.jus.br/estatistica/sead/odsele/bem_candidato/bem_candidato_2026.zip'
    return cand,bens
...
def rec(r):
    return {
      'nomeUrna':pick(r,'NM_URNA_CANDIDATO','NM_URNA_CANDIDATA'),'nomeCompleto':pick(r,'NM_CANDIDATO','NM_CANDIDATA'),
...
def main():
    cand_url,bens_url=resources()
    rb=requests.get(bens_url,headers=UA,timeout=180); rb.raise_for_status()
    assets=defaultdict(float); asset_count=defaultdict(int)
    for txt in csv_texts(rb.content):
      for r in rows(txt):
    ...
    rc=requests.get(cand_url,headers=UA,timeout=180); rc.raise_for_status()
    database=[]; total_rows=0
    for txt in csv_texts(rc.content):
      for r in rows(txt):
        total_rows+=1; x=rec(r)
        if not in_scope(x):continue
        x['patrimonio']=round(assets.get(x['sqCandidato'],0.0),2); x['qtdBens']=asset_count.get(x['sqCandidato'],0)
        database.append(x)
    ...
    payload={'source':'Tribunal Superior Eleitoral — Dados Abertos','dataset':DATASET,'candidateResource':cand_url,'assetsResource':bens_url,
      ...
      'notFound':[n for n in mon if n not in found]}
    OUT.write_text(json.dumps(payload,ensure_ascii=False,separators=(',',':'))+'\n',encoding='utf-8')
    print(f'TSE: {len(database)} candidatos no banco; patrimônio agregado; {len(found)}/{len(mon)} monitorados localizados')
