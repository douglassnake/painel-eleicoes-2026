...
def match_score(key, urna, full, rec):
...
def main():
    ...
    response = requests.get(RESOURCE_URL, timeout=180, headers={"User-Agent": "painel-eleicoes-2026/1.0"})
    ...
    for _, text in csv_texts(response.content):
        for row in parse_rows(text):
            ...
            if not urna and not full:
                continue
            rec = make_record(row)
            for key, display in targets.items():
                score = match_score(key, urna, full, rec)
