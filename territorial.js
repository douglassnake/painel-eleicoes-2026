(() => {
  ...
  function renderRanking(rows){
    ...
    return `<section class="territorial-block"><div class="section-title territorial-title"><div><h2>Ranking territorial regional</h2><p>Índice histórico de presença eleitoral no Noroeste de Minas e Alto Paranaíba. Não é pesquisa nem previsão de voto.</p></div><span class="model-pill">ITR • 0–100</span></div>
      <div class="territorial-method"><strong>Composição do ITR:</strong> 45% participação dos votos nas duas regiões, 30% volume regional, 15% alcance municipal e 10% dispersão. O histórico de 2022 é priorizado quando disponível; 2024 entra como evidência alternativa. No ${safe(D.meta.model)}, o ITR produz apenas um ajuste conservador na média simulada — ele não substitui votação partidária, quociente eleitoral ou distribuição real de cadeiras.</div>
  ...
  function renderCards(rows){
    return `<div class="section-title territorial-subtitle"><div><h3>Candidaturas regionais</h3><p>O ITR permanece visível como indicador histórico. A probabilidade abaixo vem do ${safe(D.meta.model)}, que usa o ITR somente como ajuste territorial moderado.</p></div></div>
  ...
  function drawCards(rows){
    ...
    target.innerHTML=a.map(x=>`<article class="card candidate-card territorial-card"><div class="card-head"><div class="identity"><div class="avatar large">${safe(x.nome.split(/\s+/).slice(0,2).map(s=>s[0]).join('').toUpperCase())}</div><div><div class="name">${safe(x.nome)}</div><div class="meta">${safe(x.cargo)} • ${safe(x.partido)}</div><span class="badge">${safe(x.base)}</span></div></div><div class="number">${safe(x.numero)}</div></div>
      ...
      <div class="prob"><div class="prob-head"><span>${safe(D.meta.model)}</span><strong>${x.prob.toFixed(1)}%</strong></div><div class="bar"><i class="${probClass(x.prob)}" style="width:${x.prob}%"></i></div></div>
      ${x.hist?`<p class="status">Concentração Top 3: ${pct(Number(x.hist.concentracaoTop3||0))}. A probabilidade é uma estimativa condicionada a parâmetros e cortes simplificados; não é pesquisa eleitoral nem simulação integral do sistema proporcional.</p>`:''}</article>`).join('')||'<div class="empty">Nenhum candidato encontrado.</div>';
  }
